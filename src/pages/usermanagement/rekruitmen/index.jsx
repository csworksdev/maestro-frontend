...
const educationOptions = Object.entries(educationLabels).map(
  ([value, label]) => ({
    value,
    label,
  }),
...
const getOptionId = (...values) => {
  const value = values.find((item) => {
    ...
    if (typeof item === "object") {
      return Boolean(
        item.department_id || item.branch_id || item.job_id || item.id,
      );
    }
    return true;
  });
...
const RecruitmentBadge = ({
  ...
  >
    {icon ? <Icon icon={icon} className="h-3.5 w-3.5 flex-none" /> : null}
...
const FilterCheckboxGroup = ({ group, values, disabled, onToggle }) => (
...
const Rekruitmen = () => {
  ...
  const fetchInitialFilters = useCallback(async (signal) => {
    try {
      ...
      const [departmentResult, branchResult, jobResult] =
      ...
      if (departmentResult.status === "fulfilled") {
        ...
        console.error(
          "Error fetching recruitment departments",
      ...
      if (branchResult.status === "fulfilled") {
        ...
        console.error(
          "Error fetching recruitment branches",
          branchResult.reason,
        );
      }
  ...
  const fetchData = useCallback(
    async (page, size, query, departmentId, nextFilters, signal) => {
      if (!departmentId) {
      ...
      try {
        setIsLoading(true);
        const res = await getApplicationsAll(
          normalizeParams({
            page: page + 1,
            page_size: size,
            search: query,
    ...
    async (application, status) => {
      ...
      const result = await Swal.fire({
        title: isApproved
          ...
          : "Tolak pelamar?",
        text: isStarting ? "Pelamar akan masuk ke tahap Cek CV." : undefined,
        input: "textarea",
  ...
  return (
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-[276px_minmax(0,1fr)] 2xl:grid-cols-[288px_minmax(0,1fr)] xl:items-start">
      <Card
        bodyClass="p-0"
        className="overflow-hidden xl:sticky xl:top-5 xl:self-start"
      >
        ...
        <div className="space-y-3 px-4 py-4">
          ...
          <div className={filterPanelClass}>
            ...
            <FilterSelect
              label="Tahapan"
              placeholder={
                isStageLoading ? "Memuat tahapan..." : "Semua tahapan"
              }
