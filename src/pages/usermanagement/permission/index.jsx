...
import { extractList, hasAnyPermission } from "@/utils/accessControl";
...
const extractResponseList = (response, resourceKey) => {
  ...
  if (Array.isArray(payload?.data?.data?.results))
...
const getPermissionAction = (permission) => {
  ...
  return (
    PERMISSION_ACTIONS.find((action) => code.startsWith(`${action.key}_`))
      ?.key || "other"
  );
};
...
const getPermissionModule = (permission) => {
  ...
  return action === "other"
    ? "other"
    : code.replace(`${action}_`, "") || "other";
};

const getPermissionKey = (permission) =>
  String(
    permission?.id ?? getPermissionRawCode(permission) ?? permission?.name,
  );
...
const normalizePermissionItem = (permission) => {
  ...
  return {
    ...permission,
    content_type:
      permission?.content_type || permission?.content_type_model || module,
    id,
    code,
    action,
    module,
...
const loadAllPermissionCatalog = async ({
  startPage = 1,
  initialItems = [],
} = {}) => {
...
const Permissions = () => {
  ...
  const selectedRole = useMemo(
    () =>
      roles.find(
        (role) => normalizeRoleId(role) === String(selectedRoleId),
      ) || {
        id: selectedRoleId,
  ...
  const allPermissions = useMemo(
    () =>
      catalog.map(normalizePermissionItem).filter((permission, index, list) => {
        const code = getPermissionCode(permission);
        return (
          list.findIndex((item) => getPermissionCode(item) === code) === index
        );
      }),
    [catalog],
  );
  ...
  const resolveRolePermissionIds = useCallback(
    async (roleId, normalizedCatalog) => {
      if (!roleId) {
        return [];
      }
    ...
    const loadData = async () => {
      ...
      try {
        ...
        loadAllPermissionCatalog({
          ...
          .then((catalogResult) => {
            if (!isMounted) return;
            const completeCatalog = catalogResult.items.map(
        ...
        if (isMounted) {
          setLoadError(
            getErrorMessage(
              error,
              "Gagal mengambil data permission dari backend.",
            ),
          );
          setRoles([]);
          setCatalog([]);
  ...
  useEffect(() => {
    ...
    setSelectedRoleId((currentRoleId) => {
      if (
        currentRoleId &&
        roles.some((role) => normalizeRoleId(role) === currentRoleId)
      ) {
  ...
  const refreshRolePermissions = useCallback(
    async (roleId = selectedRoleId, normalizedCatalog = catalog) => {
      ...
      try {
        const nextIds = await resolveRolePermissionIds(
          roleId,
          normalizedCatalog,
        );
  ...
  const handlePermissionSubmit = async (event) => {
    ...
    if (!payload.name || !payload.codename || !payload.content_type) {
      Swal.fire(
        "Data belum lengkap",
        "Name, codename, dan content_type wajib diisi.",
        "warning",
      );
      return;
    }
    ...
    try {
      if (editingPermissionId) {
        ...
        Swal.fire(
          "Updated!",
          "Permission berhasil diperbarui di backend.",
          "success",
        );
      } else {
        ...
        Swal.fire(
          "Created!",
          "Permission berhasil dibuat di backend.",
          "success",
        );
      }
  ...
  const handlePermissionDelete = async (permission) => {
    ...
    try {
      ...
      if (String(editingPermissionId) === String(permissionId))
        resetPermissionForm();
      Swal.fire(
        "Deleted!",
        "Permission berhasil dihapus dari backend.",
        "success",
      );
    } catch (error) {
      Swal.fire(
  ...
  const handleSavePermissions = async () => {
    ...
    try {
      await syncRolePermissionsByRoleId(
        selectedRoleId,
        selectedIds
          .map(normalizePermissionIdForPayload)
          .filter((id) => id !== null),
      );
      ...
      Swal.fire(
        "Saved!",
        "Permission berhasil disimpan ke backend.",
        "success",
      );
    } catch (error) {
  ...
  return (
    <div className="grid grid-cols-1">
      <Card
        ...
        subtitle={"Atur permission per role langsung dari data backend."}
        headerslot={
          ...
          <aside className="min-w-0 border-b border-slate-200 p-4 dark:border-slate-700 sm:p-5 xl:border-b-0 xl:border-r">
            ...
            <div className="mt-5 rounded border border-slate-200 p-4 dark:border-slate-700">
              ...
              >
                ...
                {roles.map((role) => (
                  <option
                    key={normalizeRoleId(role)}
                    value={normalizeRoleId(role)}
                  >
                    {getRoleName(role)}
                  </option>
                ))}
              ...
              <div className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                Role ini menjadi dasar data permission yang diambil dari
                backend.
              </div>
            </div>
              ...
              <>
                <div className="mb-4 flex flex-col gap-2 rounded border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <span className="ml-2">
                      {selectedIds.length}/{allPermissions.length} permission
                      aktif
                    </span>
                  </div>
                      ...
                      >
                        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                          ...
                          <button
                            ...
                            disabled={isPermissionInteractionDisabled}
                            onClick={() =>
                              handleToggleModule(group, !allChecked)
                            }
