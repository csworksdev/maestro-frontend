...
import {
  AddMenus as createMenuCatalog,
  DeleteMenus as deleteMenuCatalog,
  EditMenus as updateMenuCatalog,
  getMenusAll as getMenuCatalog,
} from "@/axios/userManagement/menu";
...
const containsNodeId = (item, targetId) =>
  toArray(item?.children).some(
    (child) =>
      getMenuId(child) === String(targetId) || containsNodeId(child, targetId),
  );
...
const buildMenuPayload = (form, overrides = {}) => {
  ...
  return {
    ...
    icon:
      menuType === "DIVIDER" ? null : String(form.icon || "").trim() || null,
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="truncate text-xs text-slate-500 dark:text-slate-400">
                  {item.menu_type || "LINK"} -{" "}
                  {item.route || item.code || "no route"} - #{item.sort_order}
                </div>
              </div>
...
const Menus = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const message = getErrorMessage(
        error,
        "Gagal mengambil menu dari backend.",
      );
  ...
  const handleSubmit = async (event) => {
    ...
    const currentNode = editingMenuId
      ? findMenuNode(menuTree, editingMenuId)
      : null;
    const sortOrder =
  ...
  const handleDragEnd = (result) => {
    ...
    setMenuTree((items) =>
      moveMenuNode(items, result.source, result.destination),
    );
    setIsDirty(true);
  };
  ...
  const handleSaveOrder = async () => {
    ...
    try {
      ...
      Swal.fire(
        "Saved!",
        "Urutan menu berhasil disimpan ke backend.",
        "success",
      );
      await fetchData();
    } catch (error) {
              ...
              >
                ...
                <div>
                  ...
                  <input
                    ...
                    disabled={form.menu_type === "DIVIDER"}
                    onChange={(event) =>
                      handleFormChange("route", event.target.value)
                    }
                  />
                </div>
                <div>
