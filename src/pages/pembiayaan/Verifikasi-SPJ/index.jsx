...
import {
  getSPJTerverifikasi,
  getVerifikasiSPJ,
} from "../components/axios/verifikasi-spj";
...
const VerifikasiSPJ = () => {
  const [listDataVerifikasiSPJ, setListDataVerifikasiSPJ] = useState([]);
  const [listDataSPJTerverifikasi, setListDataSPJTerverifikasi] = useState([]);
  const [isLoading, setIsLoading] = useState(true);

  const loadData = () => {
    getVerifikasiSPJ()
      .then((res) => {
        setListDataVerifikasiSPJ(res.data.data);
      })
      .finally(() => setIsLoading(false));
    getSPJTerverifikasi().then((res) => {
      setListDataSPJTerverifikasi(res.data.data);
    });
  };
  ...
  return (
    <div className="grid grid-cols-1">
      <Card title="Verifikasi Surat Pertanggungjawaban (SPJ)">
        <Tab.Group>
          ...
          {isLoading ? (
            ...
            <Tab.Panels>
              <Tab.Panel>
                <Verifikasi listData={listDataVerifikasiSPJ} />
              </Tab.Panel>
              <Tab.Panel>
                <TerVerifikasi listData={listDataSPJTerverifikasi} />
              </Tab.Panel>
