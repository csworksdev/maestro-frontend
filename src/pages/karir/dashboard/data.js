...
export const percent = (value, total) =>
  total > 0 ? Math.min(100, (number(value) / total) * 100) : 0;
...
const LABELS = {
  ...
  employee_referral: "Referensi karyawan",
  friend: "Teman",
  family: "Keluarga",
  other: "Lainnya",
  no_experience: "0 tahun",
...
export const labelFor = (value) =>
  LABELS[value] ||
  (/^(sma|smk|[ds][1-4])$/.test(value)
    ? value.toUpperCase()
    : String(value || "Tidak diketahui")
        .replace(/_/g, " ")
...
export const distribution = (rows, field = "value", defaults = []) => {
  ...
  asArray(rows)
    ...
    .forEach((row) => {
      ...
      grouped.set(value, {
        value,
        label: row.label || labelFor(value),
  ...
  return [
    ...defaults.map((item) => ({
      ...item,
      total: grouped.get(item.value)?.total || 0,
    })),
    ...[...grouped.values()].filter(
      (item) => !defaults.some((entry) => entry.value === item.value),
    ),
  ].map((item, index) => ({
    ...item,
...
export const readList = (payload, key) => {
  ...
  if (payload?.data && payload.data !== payload)
    return readList(payload.data, key);
