...
const ALWAYS_ALLOWED_PATHS = new Set(["", "dashboard", "notifications"]);
...
export const canAccessPath = ({
  ...
  if (
    ALWAYS_ALLOWED_PATHS.has(normalizedPath) ||
    ALWAYS_ALLOWED_PATHS.has(firstSegment)
  ) {
    return true;
  }
  ...
  const hasPermissionAccess =
    hasPermissionData &&
    hasAnyPermission(permissionCodes, permissionCandidates);

  if (hasMenuAccess) {
    if (allowMenuAccessOnly) return true;
    return (
      !hasPermissionData || !permissionCandidates.length || hasPermissionAccess
    );
  }
