...
export const ROLE_MENU_PREVIEW_STORAGE_EVENT =
  "maestro-role-menu-preview-updated";
...
const sidebarItemToNode = (item, roleId, index) => {
  ...
  return {
    ...
    code: slugify(link || title),
    menu_type:
      item.menu_type || (item.isGroup || item.child?.length ? "GROUP" : "LINK"),
...
const hydrateOriginalRoutesFromCatalog = (
  items = [],
  ...
  (items || []).map((item) => {
    ...
    return {
      ...item,
      original_route: shouldUseCatalogRoute
        ? catalogRoute
        : item.original_route,
      children: hydrateOriginalRoutesFromCatalog(
        item.children || [],
        menuRouteMap,
      ),
    };
